...
<div>
  ...
  <div class="card archive" v-for="request in filteredData" :key="request.id">
    <div class="card-header pt-2 pb-2 ps-3 pe-3">
    <h4  v-html="highlightText('Имя: '+request.client_name)"></h4>
    <button class="btn btn-outline-secondary" @click="addToActive(request.id, request.service_id, request.client_name, request.client_phone, request.message)" title="Перенести в активные"><i class="fas fa-arrow-left me-2"></i>Перенести в активные</button>
    </div>
...
export default {
  ...
  methods: {
      ...
      addToActive(id, service_id, client_name, client_phone, message) {
        axios.post('/api/requests/' + id, {
          _method: 'PUT',
          archive: 0, 
